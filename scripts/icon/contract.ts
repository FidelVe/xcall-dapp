...
export class Contract {
  ...
  async getTxResult(txHash: string) {
    ...
    for (let i = 0; i < 5; i++) {
      const result = await this.iconService
  ...
  async getBlock(
    ...
    if (param === undefined || param === "latest") {
      ...
      const height = BigNumber.isBigNumber(param)
        ? param
        : new BigNumber(param as number);
      // @ts-ignore
  ...
  async queryFilter(
    ...
    if (fromBlockOrBlockhash === toBlock || !toBlock) {
      ...
      if (
        typeof fromBlockOrBlockhash === "number" &&
        fromBlockOrBlockhash < 0
      ) {
        from = await this.getBlock(to.height + fromBlockOrBlockhash);
      } else if (
        BigNumber.isBigNumber(fromBlockOrBlockhash) &&
        fromBlockOrBlockhash.toNumber() < 0
      ) {
      ...
      for (let height = from.height + 1; height < to.height; height++) {
        ...
        const eventLogs = await this.filterEventFromBlock(
          block,
          sig,
          this.address
        );
  ...
  async waitEvent(sig: string) {
    ...
    while (true) {
      while (height < latest.height) {
        const events = await this.filterEventFromBlock(
          block,
          sig,
          this.address
        );
